...
| Desktop app | 18% | Bounded sensitive-workstation foundation now renders local runtime request-preparation modes and runtime-shaped response panes for CSV, XLSX, and PDF review with honest disclosures; actual runtime submission, file picker upload/download UX, vault browsing, decode, audit investigation, and full review workflows remain unimplemented. |
| Overall | 41% | Core workspace, vault MVP, tabular path, bounded DICOM/PDF/runtime slices, conservative media/FCS domain models, adapter/application review foundation, bounded runtime metadata review and PDF review entries, browser tabular/PDF review surface, desktop request-preparation/response workbench foundation, and local CLI foundations are present; major workflow depth and surface parity remain missing; scope-drift controller/moat CLI wording is not counted as core product progress. |
...
- `mdid-browser` is no longer only a scaffold: it now provides a local-first browser page for a bounded tabular de-identification flow that submits CSV text or base64-transported XLSX workbook bytes plus explicit field policies to the local `mdid-runtime` on localhost and renders the bounded rewritten result, summary, and review queue that come back; it also provides a bounded PDF review mode backed by `/pdf/deidentify` that submits base64 PDF bytes plus a source name and renders the review-only summary, page statuses, and review queue
- `mdid-browser` still does not provide OCR, visual redaction, handwriting handling, PDF rewrite/export, browser upload UX, desktop PDF flow, auth/session handling, generalized workflow orchestration, a generalized workflow builder, or any broader browser governance workspace
- `mdid-desktop` now renders a bounded sensitive-workstation foundation for preparing local runtime CSV, XLSX, and PDF review requests with endpoint previews, validation status, mode-specific disclosures, and local runtime-shaped summary, review queue, rewritten-output/review-notice, and error panes; it still does not implement actual runtime submission, file picker upload/download UX, vault browsing, decode, audit investigation, OCR, visual redaction, PDF rewrite/export, controller workflows, or full review workflows
