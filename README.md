...
Completion snapshot, based only on landed repository features and verification state (truth-synced 2026-04-29 after bounded browser vault audit browsing landed in commit `480b2cc`):
...
| Browser/web | 44% | Bounded localhost tabular de-identification page plus bounded PDF review mode, DICOM tag-level helper, metadata-only conservative image/video/FCS review JSON mode, and one real read-only vault audit event browsing request/submission surface backed by the existing local `/vault/audit/events` runtime route, with bounded CSV/XLSX/PDF/DICOM file import/export helper controls and JSON metadata import/report export controls; the vault audit mode browses audit events only and is not vault decode/export, not a broader governance workspace, and not a media-byte upload/rewrite surface. |
