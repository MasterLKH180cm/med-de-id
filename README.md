...
|---|---:|---|
| CLI | 72% | Early automation surface with local de-identification readiness, bounded `deidentify-csv`, `deidentify-xlsx`, and `deidentify-dicom` commands for local file rewriting through explicit policies and the local application/vault stack, plus a bounded `deidentify-pdf` command that writes a PHI-safe review report only; vault/decode, audit, conservative-media, and broader import/export CLI commands remain unimplemented; active moat/controller CLI behavior has been removed from the product surface rather than counted as completion. |
