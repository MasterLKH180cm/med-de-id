...
| Desktop app | 26% | Bounded sensitive-workstation foundation prepares CSV, XLSX, and PDF review requests, can apply bounded CSV/XLSX/PDF file import/export helpers, submit prepared envelopes to a localhost runtime, and render response panes with honest disclosures; vault browsing, decode, audit investigation, PDF rewrite/export, and full review workflows remain unimplemented. |
| Overall | 45% | Core workspace, vault MVP, tabular path, bounded DICOM/PDF/runtime slices, conservative media/FCS domain models, adapter/application review foundation, bounded runtime metadata review and PDF review entries, browser tabular/PDF review surface with bounded CSV/XLSX/PDF import/export helpers, desktop request-preparation/localhost-submit/response workbench foundation with bounded CSV/XLSX/PDF file import/export helpers, and local CLI foundations are present; major workflow depth and surface parity remain missing; unrelated scope-drift legacy CLI commands are not counted as core product progress. |

Missing items include deeper policy/detection crates, full review/governance workflows, richer browser UX including deeper upload/download UX beyond bounded CSV/XLSX/PDF import/export helpers, deeper desktop vault/decode/audit workflows, desktop PDF flow beyond request preparation and bounded review/export helper naming, broader import/export and upload flows, OCR, visual redaction, handwriting handling, full PDF rewrite/export, FCS semantic parsing, media rewrite/export, generalized spreadsheet handling, auth/session handling where needed, removal or isolation of scope-drift legacy CLI surfaces from product-facing documentation and roadmap claims, and production packaging/hardening.
...
- `mdid-desktop` now renders a bounded sensitive-workstation foundation for preparing local runtime CSV, XLSX, and PDF review requests with endpoint previews, validation status, mode-specific disclosures, bounded CSV/XLSX/PDF file import/export helpers, localhost runtime submission, and local runtime-shaped summary, review queue, rewritten-output/review-notice, and error panes; it still does not implement deeper desktop vault/decode/audit workflows, OCR, visual redaction, PDF rewrite/export, or full review workflows
