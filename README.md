...
|---|---:|---|
| CLI | 55% | Early automation surface with local de-identification readiness and a bounded `deidentify-csv` command for local CSV rewriting through explicit field policies and the local vault stack; vault/decode, audit, and import/export foundations remain primarily in library/runtime layers; active moat/controller CLI behavior has been removed from the product surface rather than counted as completion. |
...
- `mdid-cli` remains an early de-identification automation surface and now includes a bounded `deidentify-csv` command that reads a local CSV, applies explicit field policies through the application/vault stack, writes a rewritten local CSV, and prints a PHI-safe JSON summary. Active moat/controller CLI behavior has been removed from the product surface because agent workflow / controller loop / planner-coder-reviewer coordination semantics are scope drift for med-de-id; future CLI work should expose only de-identification workflows such as additional local tabular runs, vault/decode, audit, import/export, and verification.
