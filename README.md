...
Completion snapshot, based only on landed repository features and verification state (truth-synced 2026-04-29 after browser PHI-safe structured media review JSON downloads landed and were verified):

| Area | Completion | Status |
|---|---:|---|
| CLI | 95% | Automation surface remains unchanged this round: local de-identification readiness, bounded CSV/XLSX/DICOM rewrite commands, PDF review report only, conservative media metadata review, vault audit/decode, portable export/import/inspect, and metadata-only artifact verification are landed. No PDF rewrite/export, OCR/visual redaction, generalized transfer workflow UX, controller/agent/orchestration platform behavior, or packaging/hardening is claimed. |
...
| Desktop app | 64% | Desktop remains unchanged this round: it includes a PHI-safe helper-layer structured JSON download for already-rendered bounded PDF review reports, with strict summary/review-queue allowlists and no raw PDF bytes/debug text. Existing desktop work also covers request preparation/submission for bounded CSV/XLSX/PDF-review/DICOM/media/vault/portable routes, PHI-safe response summaries, portable artifact JSON handoff/save, explicit portable artifact filename detection, audit limit parity, helper-layer import/export naming, and save support for already-received rewritten CSV, XLSX, and DICOM runtime outputs. This is not a full desktop file picker/save workflow, and it does not claim PDF rewrite/export, OCR, visual redaction, vault browsing, decoded-value display, media review report downloads, auth/session handling, or agent/controller/orchestration behavior. |
| Overall | 92% | The repo has a local-first workspace, vault MVP, tabular path, bounded DICOM/PDF-review/runtime slices, conservative media/FCS metadata review foundation, browser helper surfaces including binary rewritten XLSX/DICOM output downloads and structured PHI-safe PDF/media/portable/vault review report JSON downloads, CLI automation, portable artifact commands/surfaces, bounded desktop output-save helper support for already-received rewritten CSV/XLSX/DICOM runtime artifacts, and bounded desktop PHI-safe PDF review report JSON downloads. Missing >=95% blockers remain explicit: full desktop file picker/save UX depth, richer browser/desktop workflows, desktop media review report downloads, OCR/visual redaction, deeper vault UX, full portable transfer workflow UX, media-byte rewrite/export, PDF rewrite/export, auth/session where needed, packaging/hardening, and broader governance/review polish. |

Verification evidence for this truth-sync: Browser media review report JSON download verification passed with `cargo test -p mdid-browser media_review_download_is_structured_and_phi_safe -- --nocapture`, `cargo test -p mdid-browser --lib`, `cargo clippy -p mdid-browser --all-targets -- -D warnings`, and `git diff --check`, plus independent spec and quality reviews for the landed browser media download slice. Earlier desktop PDF review report JSON download verification remains as previously recorded with desktop native tests/clippy and review.
