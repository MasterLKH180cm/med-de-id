...
Completion snapshot, based only on landed repository features and verification state (truth-synced 2026-04-29 after desktop PDF review report JSON downloads landed and were verified):
...
| Browser/web | 72% | Browser surface remains unchanged this round: bounded localhost tabular/PDF-review/DICOM helper flows, metadata-only conservative media JSON review, vault audit/decode request surfaces, portable export/inspect/import request/response surfaces, encrypted portable artifact JSON download for vault export, bounded file import/export helpers, real binary browser downloads for already-received rewritten XLSX and DICOM runtime outputs, and structured PHI-safe JSON downloads for already-rendered browser review/metadata response modes are landed. Generalized upload/download depth, richer browser workflows, full vault browsing, auth/session handling, OCR/visual redaction, media rewrite/export, and PDF rewrite/export remain unimplemented. |
| Desktop app | 64% | Desktop now adds a PHI-safe helper-layer structured JSON download for already-rendered bounded PDF review reports, with strict summary/review-queue allowlists and no raw PDF bytes/debug text. Existing desktop work also covers request preparation/submission for bounded CSV/XLSX/PDF-review/DICOM/media/vault/portable routes, PHI-safe response summaries, portable artifact JSON handoff/save, explicit portable artifact filename detection, audit limit parity, helper-layer import/export naming, and save support for already-received rewritten CSV, XLSX, and DICOM runtime outputs. This is not a full desktop file picker/save workflow, and it does not claim PDF rewrite/export, OCR, visual redaction, vault browsing, decoded-value display, media review report downloads, auth/session handling, or agent/controller/orchestration behavior. |
| Overall | 91% | The repo has a local-first workspace, vault MVP, tabular path, bounded DICOM/PDF-review/runtime slices, conservative media/FCS metadata review foundation, browser helper surfaces including binary rewritten XLSX/DICOM output downloads and structured review report JSON downloads, CLI automation, portable artifact commands/surfaces, bounded desktop output-save helper support for already-received rewritten CSV/XLSX/DICOM runtime artifacts, and bounded desktop PHI-safe PDF review report JSON downloads. Missing >=95% blockers remain explicit: full desktop file picker/save UX depth, richer browser/desktop workflows, desktop media review report downloads, OCR/visual redaction, deeper vault UX, full portable transfer workflow UX, media-byte rewrite/export, PDF rewrite/export, auth/session where needed, packaging/hardening, and broader governance/review polish. |

Verification evidence for this truth-sync: Desktop PDF review report JSON download verification passed with `cargo test -p mdid-desktop review_report_download -- --nocapture`, `cargo test -p mdid-desktop --lib`, `cargo clippy -p mdid-desktop --all-targets -- -D warnings`, and `git diff --check`, plus independent spec and quality reviews after the sanitizer was tightened to fail closed on arbitrary summary keys and enum-like PHI values. Earlier browser structured review report JSON download verification remains as previously recorded with native browser tests/clippy; a wasm-target check remains blocked by pre-existing workspace `uuid` wasm randomness configuration outside that browser slice, so browser claims stay limited to controller-verified native tests and reviewed wasm download code.
