...
|---|---:|---|
| CLI | 62% | Early automation surface with local de-identification readiness and bounded `deidentify-csv` plus `deidentify-xlsx` commands for local tabular rewriting through explicit field policies and the local vault stack; vault/decode, audit, PDF, DICOM, conservative-media, and import/export CLI commands remain unimplemented; active moat/controller CLI behavior has been removed from the product surface rather than counted as completion. |
...
- `mdid-cli` remains an early de-identification automation surface and now includes bounded `deidentify-csv` and `deidentify-xlsx` commands that read a local tabular file, apply explicit field policies through the application/vault stack, write local de-identified output, and print a PHI-safe JSON summary. CSV output is rewritten CSV. XLSX CLI output is a normalized single-sheet XLSX generated from de-identified tabular rows; it does not preserve original workbook metadata, formatting, formulas, sheet names, or multiple sheets. Active moat/controller CLI behavior has been removed from the product surface because agent workflow / controller loop / planner-coder-reviewer coordination semantics are scope drift for med-de-id.
