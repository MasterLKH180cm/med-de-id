...
Completion snapshot, based only on landed repository features and verification state (truth-synced 2026-04-29 after desktop vault/portable runtime submission wiring landed in commits `75e9bb8` and `5cde7e7`, with `mdid-desktop` tests and clippy passing):
...
| Browser/web | 49% | Bounded localhost tabular de-identification page plus bounded PDF review mode, DICOM tag-level helper, metadata-only conservative image/video/FCS review JSON mode, one real read-only vault audit event browsing request/submission surface backed by the existing local `/vault/audit/events` runtime route, and bounded browser vault decode request mode backed by localhost `/vault/decode` that submits explicit record ids and renders only decoded counts/disclosure with decoded PHI hidden in the browser pane; bounded CSV/XLSX/PDF/DICOM file import/export helper controls and JSON metadata import/report export controls are present, but vault browsing, browser vault export/import, raw decoded value rendering, broader governance workflow, and media-byte upload/rewrite remain unimplemented. |
...
| Overall | 77% | Core workspace, vault MVP, tabular path, bounded DICOM/PDF/runtime slices, conservative media/FCS domain models, adapter/application review foundation, bounded runtime metadata review/PDF review/DICOM/vault decode/audit/portable export/import entries, browser tabular/PDF review/DICOM helper surface with bounded CSV/XLSX/PDF/DICOM import/export helpers, metadata-only conservative media JSON review, read-only vault audit event browsing through `/vault/audit/events`, and explicit-record browser vault decode through localhost `/vault/decode` with PHI-hidden browser rendering are present, alongside desktop request-preparation/localhost-submit/response workbench foundation with bounded CSV/XLSX/PDF/DICOM file import/export helpers, metadata-only conservative media JSON review preparation/submission and PHI-safe response rendering, and desktop vault/portable localhost submission wiring for bounded vault decode/audit and portable export/inspect/import responses through PHI-safe summaries/notices, plus CLI automation for CSV/XLSX/DICOM/PDF/conservative-media/vault audit/decode/portable import/export; missing >90% gaps remain for deeper detection/policy, richer browser/desktop workflows, OCR/visual redaction, media-byte rewrite/export, browser vault export/import, deeper vault UX, full desktop vault/decode/audit execution UX, governance/review polish, file picker/upload-download depth, and packaging/hardening. |

Missing items include distance to >90% completion: deeper detection/policy, richer browser/desktop workflows, OCR/visual redaction, browser vault export/import, deeper vault UX, full desktop vault/decode/audit execution UX, file picker/upload-download depth, governance/review polish, media-byte rewrite/export, packaging/hardening, deeper policy/detection crates, full review/governance workflows, richer browser UX and upload/download depth beyond the bounded CSV/XLSX/PDF/DICOM import/export helpers, metadata-only JSON media review, deeper desktop vault browsing, desktop decode workflow execution UX, audit investigation/execution workflow polish, generalized portable transfer workflow UX, desktop PDF flow beyond request preparation and bounded review/export helper naming, desktop DICOM flow beyond bounded request/response/import/export helper support, broader import/export and upload flows, handwriting handling, full PDF rewrite/export, FCS semantic parsing, generalized spreadsheet handling, auth/session handling where needed, and final verification/audit polish across surfaces. Remaining CLI gaps include conservative-media rewrite/export, generalized transfer workflows, verification/audit polish, richer workflows, and production packaging/hardening.
