...
| Desktop app | 33% | Bounded sensitive-workstation foundation prepares CSV, XLSX, PDF review, DICOM, and bounded vault decode/audit request envelopes for existing localhost runtime routes, can apply bounded CSV/XLSX/PDF/DICOM file import/export helpers, submit prepared non-vault envelopes to a localhost runtime, and render response panes with honest disclosures; deeper desktop vault browsing, decode workflow execution UX, audit investigation workflow, portable transfer UX, OCR, visual redaction, PDF rewrite/export, full DICOM review workflow, and full review workflows remain unimplemented. |
| Overall | 48% | Core workspace, vault MVP, tabular path, bounded DICOM/PDF/runtime slices, conservative media/FCS domain models, adapter/application review foundation, bounded runtime metadata review/PDF review/DICOM/vault decode/audit entries, browser tabular/PDF review surface with bounded CSV/XLSX/PDF import/export helpers, desktop request-preparation/localhost-submit/response workbench foundation with bounded CSV/XLSX/PDF/DICOM file import/export helpers and bounded desktop vault decode/audit request-preparation helpers, and local CLI foundations are present; major workflow depth and surface parity remain missing; unrelated scope-drift legacy CLI commands are not counted as core product progress. |
...
- `mdid-desktop` now renders a bounded sensitive-workstation foundation for preparing local runtime CSV, XLSX, PDF review, and DICOM requests with endpoint previews, validation status, mode-specific disclosures, bounded CSV/XLSX/PDF/DICOM file import/export helpers, localhost runtime submission, and local runtime-shaped summary, review queue, rewritten-output/review-notice, and error panes; it also includes pure bounded vault decode/audit request-preparation helpers for existing localhost runtime vault routes. It still does not implement deeper desktop vault browsing, decode workflow execution UX, audit investigation workflow, portable transfer UX, OCR, visual redaction, PDF rewrite/export, desktop DICOM flow beyond bounded request/response/import/export helper support, full DICOM review workflow, or full review workflows
