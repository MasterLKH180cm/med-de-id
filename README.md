...
|---|---:|---|
| CLI | 78% | Early automation surface with local de-identification readiness, bounded `deidentify-csv`, `deidentify-xlsx`, and `deidentify-dicom` commands for local file rewriting through explicit policies and the local application/vault stack, plus a bounded `deidentify-pdf` command that writes a PHI-safe review report only, a bounded read-only `vault-audit` command that prints PHI-safe audit metadata, and a bounded `vault-decode` command that unlocks a local vault, decodes explicit record ids, writes a local JSON report containing decoded values, records an audit event, and prints only a PHI-safe summary; conservative-media, broader portable import/export CLI commands, and richer workflows remain unimplemented. |
