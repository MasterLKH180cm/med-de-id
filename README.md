...
|---|---:|---|
| CLI | 68% | Early automation surface with local de-identification readiness and bounded `deidentify-csv`, `deidentify-xlsx`, and `deidentify-dicom` commands for local file rewriting through explicit policies and the local application/vault stack; vault/decode, audit, PDF, conservative-media, and import/export CLI commands remain unimplemented; active moat/controller CLI behavior has been removed from the product surface rather than counted as completion. |
