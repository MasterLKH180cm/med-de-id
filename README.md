...
Completion snapshot, based only on landed repository features and verification state (truth-synced 2026-04-29 after browser structured review report JSON downloads landed and were verified):

| Area | Completion | Status |
|---|---:|---|
| CLI | 95% | Automation surface remains unchanged: local de-identification readiness, bounded CSV/XLSX/DICOM rewrite commands, PDF review report only, conservative media metadata review, vault audit/decode, portable export/import/inspect, and metadata-only artifact verification are landed. No PDF rewrite/export, OCR/visual redaction, generalized transfer workflow UX, controller/agent/orchestration platform behavior, or packaging/hardening is claimed. |
| Browser/web | 72% | Browser surface now includes bounded localhost tabular/PDF-review/DICOM helper flows, metadata-only conservative media JSON review, vault audit/decode request surfaces, portable export/inspect/import request/response surfaces, encrypted portable artifact JSON download for vault export, bounded file import/export helpers, real binary browser downloads for already-received rewritten XLSX and DICOM runtime outputs, and structured PHI-safe JSON downloads for already-rendered browser review/metadata response modes. Generalized upload/download depth, richer browser workflows, full vault browsing, auth/session handling, OCR/visual redaction, media rewrite/export, and PDF rewrite/export remain unimplemented. |
| Desktop app | 62% | Bounded sensitive-workstation foundation remains unchanged this round: helper-layer save support for already-received rewritten CSV, XLSX, and DICOM runtime outputs, writing only extracted artifact bytes through a PHI-safe save helper/action. Existing desktop work also covers request preparation/submission for bounded CSV/XLSX/PDF-review/DICOM/media/vault/portable routes, PHI-safe response summaries, portable artifact JSON handoff/save, explicit portable artifact filename detection, audit limit parity, and helper-layer import/export naming. This is not a full desktop file picker/save workflow, and it does not claim PDF rewrite/export, OCR, visual redaction, vault browsing, decoded-value display, auth/session handling, or agent/controller/orchestration behavior. |
| Overall | 91% | The repo has a local-first workspace, vault MVP, tabular path, bounded DICOM/PDF-review/runtime slices, conservative media/FCS metadata review foundation, browser helper surfaces including binary rewritten XLSX/DICOM output downloads and structured review report JSON downloads, CLI automation, portable artifact commands/surfaces, and bounded desktop output-save helper support for already-received rewritten CSV/XLSX/DICOM runtime artifacts. Missing >=95% blockers remain explicit: full desktop file picker/save UX depth, richer browser/desktop workflows, OCR/visual redaction, deeper vault UX, full portable transfer workflow UX, media-byte rewrite/export, PDF rewrite/export, auth/session where needed, packaging/hardening, and broader governance/review polish. |

Verification evidence for this truth-sync: Browser structured review report JSON download verification passed with `cargo test -p mdid-browser pdf_review_download_exports_structured_json_report -- --nocapture`, `cargo test -p mdid-browser portable_review_download_exports_json_without_raw_runtime_body -- --nocapture`, `cargo test -p mdid-browser output_download -- --nocapture`, `cargo test -p mdid-browser --lib`, `cargo clippy -p mdid-browser --all-targets -- -D warnings`, and `git diff --check`, plus independent spec and quality reviews. Task 2 README grep verification confirms the updated completion snapshot. A wasm-target check remains blocked by pre-existing workspace `uuid` wasm randomness configuration outside this slice, so the claim stays limited to controller-verified native tests and reviewed wasm download code.
