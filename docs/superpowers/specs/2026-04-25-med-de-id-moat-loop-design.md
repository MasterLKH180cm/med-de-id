...
- a bounded operator-facing `mdid-cli moat round [--input-path PATH] [--history-path PATH] [--format text|json]` runner over deterministic sample inputs by default or a local JSON `MoatRoundInput` file when supplied, including override flags for strategy/spec/implementation/review budgets plus `tests_passed`, canonical executed task IDs, surfaced `implemented_specs` handoff IDs, honest `stop_reason` reporting, `input_path=PATH` provenance for input-file mode, optional `--history-path PATH` persistence only when explicitly requested, default stable text output, and a pretty deterministic `moat_round` JSON envelope for external autonomous controllers that includes source/path/provenance, score delta, continuation, task-state, decision-summary, and local-only constraint fields
- a bounded operator-facing `mdid-cli moat control-plane [--input-path PATH] [--format text|json]` runner over deterministic sample inputs by default or a local JSON `MoatRoundInput` file when supplied, plus read-only `--history-path PATH` inspection of the latest persisted control-plane snapshot, including task states, ready-node visibility, bounded decision-memory summary, and inspection-only `agent_assignments` projection for ready nodes; input-file control-plane mode prints `source=input`/`input_path=PATH`, does not save history, and remains local-only; `--format` defaults to `text`, while `--format json` emits a pretty deterministic `moat_control_plane` envelope with `type`, nullable `history_path`, `source`, `round_id`, `score`, `improvement_delta`, `can_continue`, `ready_tasks`, `assignments`, `task_states`, `decision_summary`, and local-only/read-only/no-daemon/no-agent-launch constraints
- a bounded operator-facing `mdid-cli moat history --history-path PATH [--round-id ROUND_ID] [--decision Continue|Stop|Pivot] [--contains TEXT] [--stop-reason-contains TEXT] [--min-score N] [--tests-passed true|false] [--limit N] [--format text|json]` summary surface for inspecting persisted local history, including the latest surfaced `implemented_specs` handoff IDs as `latest_implemented_specs`; filters combine conjunctively across `--round-id`, `--decision`, `--contains`, `--stop-reason-contains`, `--min-score`, and `--tests-passed`; `--round-id ROUND_ID` is read-only, exact-matches the persisted `entry.report.summary.round_id`, and applies before `--limit`; `--min-score N` is read-only, accepts a non-negative integer, filters persisted entries where `entry.report.summary.moat_score_after >= N`, applies before `--limit`; `--tests-passed true|false` is read-only, accepts exact boolean values, filters persisted entries where `entry.report.summary.tests_passed` equals the requested value, applies before `--limit`; `--format` defaults to `text`; `--format json` is read-only and emits a pretty deterministic envelope with `type: "moat_history"`, `history_path`, `summary` (`total_rounds`, nullable `latest_round_id`, nullable `latest_decision`, `latest_implemented_specs`, nullable `latest_moat_score_after`, nullable `best_moat_score_after`), `history_rounds`, and `rounds` entries containing `round_id`, `decision`, `moat_score_after`, nullable `stop_reason`, `tests_passed`, and `implemented_specs`; it never runs rounds, appends history, schedules work, launches agents, opens PRs, mutates history, or creates cron jobs
- a read-only `mdid-cli moat decision-log --history-path PATH [--round-id ROUND_ID] [--role planner|coder|reviewer] [--contains TEXT] [--summary-contains TEXT] [--rationale-contains TEXT] [--limit N]` inspection surface that requires an already-persisted history file, inspects the latest persisted round when `--round-id` is absent, exact-matches persisted `entry.report.summary.round_id` when `--round-id ROUND_ID` is present, applies round selection before role/text/summary/rationale/limit filters, prints exactly `decision_log_entries=0` when no persisted round matches, and never runs or appends a new round; it optionally filters decisions by Planner/Coder/Reviewer role, a case-sensitive substring over persisted unescaped decision summary/rationale (`--contains`), a case-sensitive substring over persisted unescaped decision summary only (`--summary-contains`), and/or a case-sensitive substring over persisted unescaped decision rationale only (`--rationale-contains TEXT`), with optional positive `--limit N` applied after those filters; `--round-id`, `--role`, `--contains`, `--summary-contains`, and `--rationale-contains` combine conjunctively, and prints each persisted decision as `decision=<role>|<summary>|<rationale>` with summary/rationale escaped for pipe-delimited output (`\\`, `|`, newline, and carriage return become `\\\\`, `\\|`, `\\n`, and `\\r`)
- `mdid-cli moat assignments --history-path PATH [--round-id ROUND_ID] [--role planner|coder|reviewer] [--state pending|ready|in_progress|completed|blocked] [--kind market_scan|competitor_analysis|lock_in_analysis|strategy_generation|spec_planning|implementation|review|evaluation] [--node-id NODE_ID] [--assigned-agent-id AGENT_ID] [--depends-on NODE_ID] [--no-dependencies] [--title-contains TEXT] [--spec-ref SPEC_REF] [--contains TEXT] [--limit N]` is a read-only latest-round inspection surface for persisted `agent_assignments`. It prints bounded `assignment=<role>|<node_id>|<title>|<kind>|<spec_ref>` rows for operators and future SDD handoff tooling; persisted string fields (`node_id`, `title`, `spec_ref`) are escaped for pipe-delimited output (`\\`, `|`, newline, and carriage return become `\\\\`, `\\|`, `\\n`, and `\\r`). It inspects the latest persisted `agent_assignments` rows only from an existing history file unless `--round-id ROUND_ID` is provided; `--round-id` is read-only, exact-matches the persisted `entry.report.summary.round_id`, selects that persisted round before assignment projection, and absence preserves latest-round behavior. `--kind` accepts only exact persisted task-node kind wire values, `--node-id` uses exact persisted node ID matching, `--assigned-agent-id AGENT_ID` exact-matches persisted task-node `assigned_agent_id` for read-only inspection of tasks owned by a persisted local worker ID, `--depends-on NODE_ID` finds the persisted selected-round task-graph node whose `node_id` matches each assignment and keeps only assignments whose persisted dependency list contains the requested upstream node ID exactly, `--no-dependencies` keeps only assignments whose selected-round task-graph node has no persisted dependencies, `--title-contains` performs a case-sensitive substring match over persisted assignment titles, `--spec-ref` performs an exact match against persisted `assignment.spec_ref`, and `--contains TEXT` performs a case-sensitive substring match over raw persisted assignment `node_id`, `title`, or `spec_ref` before escaping. Filters combine conjunctively with `--role`, `--state`, `--kind`, `--node-id`, `--depends-on`, `--no-dependencies`, `--title-contains`, `--spec-ref`, and `--contains`. `--limit N` accepts positive integers only, applies after all other filters including dependency filters, and keeps the first `N` rows in deterministic persisted assignment order. No matches return `assignment_entries=0` without error. It does not mutate history, append rounds, schedule work, launch agents, crawl data, open PRs, or create cron jobs.
- `mdid-cli moat task-graph --history-path PATH [--round-id ROUND_ID] [--role planner|coder|reviewer] [--state pending|ready|in_progress|completed|blocked] [--kind market_scan|competitor_analysis|lock_in_analysis|strategy_generation|spec_planning|implementation|review|evaluation] [--node-id NODE_ID] [--assigned-agent-id AGENT_ID] [--depends-on NODE_ID] [--no-dependencies] [--title-contains TEXT] [--spec-ref SPEC_REF] [--contains TEXT] [--limit N]` is a read-only inspection surface for the persisted control-plane task graph, defaulting to the latest persisted round when `--round-id` is absent and exact-matching persisted `entry.report.summary.round_id` when provided. It opens only existing history and prints compatibility-preserving `node=<role>|<node_id>|<title>|<kind>|<state>|<dependencies>|<spec_ref>` rows plus adjacent `assigned_agent_id=<node_id>|<owner-or-<none>>` metadata lines with `<none>` for empty dependency/spec/assigned-agent fields, comma-joined dependency node IDs, optional role/state/kind filtering, optional exact persisted `--node-id` matching with no normalization, optional exact persisted `--assigned-agent-id AGENT_ID` matching against `node.assigned_agent_id` for read-only inspection of tasks owned by a persisted local worker ID, optional exact `--depends-on NODE_ID`, optional `--no-dependencies`, optional case-sensitive `--title-contains` substring matching against persisted node titles without normalization or mutation, optional `--spec-ref SPEC_REF` exact matching against raw persisted `node.spec_ref.as_deref()` without comparing escaped output, matching `<none>`, substring matching, or normalization, optional case-sensitive `--contains TEXT`, and optional positive `--limit N`. Escaping applies only to pipe-delimited output fields. Filters are conjunctive and `--limit` applies after the other filters. `--kind` accepts only exact persisted task-node kind wire values without normalization. When no persisted node matches the combined filters, it succeeds with only the `moat task graph` header and never appends, schedules, runs agents, or launches background work. It does not mutate history, append rounds, schedule work, launch agents, crawl data, open PRs, or create cron jobs.
- `mdid-cli moat ready-tasks --history-path PATH [--round-id ROUND_ID] [--role planner|coder|reviewer] [--kind market_scan|competitor_analysis|lock_in_analysis|strategy_generation|spec_planning|implementation|review|evaluation] [--node-id NODE_ID] [--depends-on NODE_ID] [--no-dependencies] [--requires-artifacts] [--title-contains TEXT] [--spec-ref SPEC_REF] [--limit N] [--format text|json]` is a bounded latest-round routing surface for autonomous controllers. It opens only an existing history file, selects the latest persisted round unless `--round-id` exact-matches a specific persisted round, derives immediately claimable nodes from `task_graph.ready_node_ids()`, applies optional round/role/kind, read-only exact persisted `--node-id NODE_ID`, read-only `--depends-on NODE_ID`, read-only `--no-dependencies`, read-only `--requires-artifacts`, read-only `--title-contains TEXT`, and read-only `--spec-ref SPEC_REF` filters conjunctively before `--limit`, with `--depends-on NODE_ID` keeping only ready nodes whose persisted dependency list contains the requested upstream node ID exactly, `--no-dependencies` keeping only ready nodes with an empty persisted dependency list, `--requires-artifacts` keeping only ready nodes whose persisted dependency node IDs all resolve to completed task nodes with at least one artifact handoff while root nodes with no dependencies pass, `--title-contains TEXT` performing a case-sensitive substring match over persisted ready node titles and `--spec-ref SPEC_REF` performing an exact case-sensitive match over persisted ready node spec references, and prints `ready_task=<role>|<kind>|<node_id>|<title>|<spec_ref>` rows without launching agents or mutating history. `--format` defaults to `text`; `--format json` is read-only and emits a pretty deterministic envelope with `type: "moat_ready_tasks"`, `round_id`, `history_path`, `ready_task_entries`, and `tasks`, where each task includes `role`, `kind`, `node_id`, `title`, and nullable `spec_ref`.
...
- `mdid-cli moat dispatch-next --history-path PATH [--round-id ROUND_ID] [--role planner|coder|reviewer] [--kind market_scan|competitor_analysis|lock_in_analysis|strategy_generation|spec_planning|implementation|review|evaluation] [--node-id NODE_ID] [--depends-on NODE_ID] [--no-dependencies] [--requires-artifacts] [--title-contains TEXT] [--spec-ref SPEC_REF] [--agent-id AGENT_ID] [--lease-seconds N] [--dry-run] [--format text|json]` is a bounded one-task dispatch envelope for external Planner/Coder/Reviewer controllers. It opens only an existing history file, selects exactly one persisted ready node in task-graph order after optional round/role/kind/node-id/dependency/artifact/title/spec-ref filters, emits deterministic task metadata plus the matching `complete-task` handoff command, and never launches agents, daemons, PRs, crawlers, or cron jobs. Non-dry-run dispatch marks the selected task `in_progress`, persists optional `assigned_agent_id`, and records a local lease deadline using `--lease-seconds N` (default 900; positive integers only). `--format` defaults to `text`; `--format json` emits a parseable envelope with `type: "moat_dispatch_next"`, `dry_run`, `claimed`, nullable request `agent_id`, nullable persisted `assigned_agent_id`, `round_id`, `node_id`, `role`, `kind`, `title`, `dependencies`, nullable `spec_ref`, and `complete_command`. Claim envelopes also include `previous_state: "ready"`, `new_state: "in_progress"`, and numeric `lease_seconds`; they intentionally do not expose `lease_expires_at` on this dispatch envelope (lease deadlines remain persisted metadata and are visible through claim/heartbeat/task-event surfaces).
...
- `mdid-cli moat claim-task --history-path PATH --node-id NODE_ID [--round-id ROUND_ID] [--agent-id AGENT_ID] [--lease-seconds N] [--format text|json]` is a bounded local coordination mutation for external Planner/Coder/Reviewer controllers. It opens only an existing history file, selects the latest persisted round unless `--round-id` exact-matches a specific persisted round, reloads the latest on-disk history before mutation to avoid stale-handle double claims, requires the selected node to be `ready`, persists only that node's state transition to `in_progress` plus optional `assigned_agent_id` local ownership metadata and a local lease deadline using `--lease-seconds N` (default 900; positive integers only), and prints stable text claim metadata by default (`round_id`, `node_id`, `assigned_agent_id`, `lease_seconds`, `previous_state=ready`, `new_state=in_progress`, and `history_path`). `--format` defaults to `text`; `--format json` emits a pretty deterministic envelope with `type: "moat_claim_task"`, `round_id`, `history_path`, `node_id`, nullable `assigned_agent_id`, `lease_seconds`, `lease_expires_at`, `previous_state`, and `new_state`. It does not run agents, append rounds, schedule work, crawl data, open PRs, or create cron jobs.
- `mdid-cli moat heartbeat-task --history-path PATH --node-id NODE_ID [--round-id ROUND_ID] [--agent-id AGENT_ID] [--lease-seconds N] [--format text|json]` is a bounded local coordination mutation for external controllers to renew an existing in-progress task lease. It opens only an existing history file, reloads latest on-disk history before mutation, requires the selected node to be `in_progress`, requires `--agent-id` to match persisted ownership when the task is already owned, records a refreshed local lease deadline using `--lease-seconds N` (default 900; positive integers only), and never launches agents, appends rounds, schedules work, crawls data, opens PRs, or creates cron jobs. `--format` defaults to `text` with unchanged stable text output; `--format json` emits a pretty deterministic envelope with `type: "moat_heartbeat_task"`, `round_id`, `history_path`, `node_id`, nullable `agent_id`, `lease_seconds`, `lease_expires_at`, and `state`.
- `mdid-cli moat reap-stale-tasks --history-path PATH [--round-id ROUND_ID] [--now RFC3339] [--format text|json]` is a bounded local coordination mutation for external controllers to release expired in-progress task leases. It opens only an existing history file, selects the latest persisted round unless `--round-id` exact-matches a specific persisted round, compares persisted lease deadlines against `--now RFC3339` when supplied or the current local time otherwise, requeues only expired locally leased `in_progress` tasks to `ready`, clears their lease/owner metadata, prints deterministic reaped counts/node IDs, and never launches agents, appends rounds, schedules work, crawls data, opens PRs, or creates cron jobs. `--format` defaults to `text` with unchanged stable text output; `--format json` emits a pretty deterministic envelope with `type: "moat_reap_stale_tasks"`, `round_id`, `history_path`, `reaped_count`, and `reaped_node_ids`.
- `mdid-cli moat complete-task --history-path PATH --node-id NODE_ID [--round-id ROUND_ID] [--agent-id AGENT_ID] [--artifact-ref TEXT --artifact-summary TEXT] [--format text|json]` is a bounded local coordination mutation for external Planner/Coder/Reviewer controllers. It opens only an existing history file, selects the latest persisted round unless `--round-id` exact-matches a specific persisted round, reloads latest on-disk history before mutation, requires the selected node to be `in_progress`, requires optional `--agent-id` to match persisted ownership when the task is already owned, persists only that task transition to `completed`, records the completion event with the assigned or supplied agent identity, and optionally records a paired artifact handoff on the completed task when both `--artifact-ref` and `--artifact-summary` are supplied. Its deterministic text output includes `artifact_recorded=true|false`, `artifact_ref=<none>|...`, and `artifact_summary=<none>|...` before downstream routing rows, then reloads the selected persisted round and prints `next_ready_task_entries=N` plus `next_ready_task=<role>|<node_id>|<title>|<kind>|<spec_ref>` for any newly/currently ready nodes exposed by dependency resolution. `--format` defaults to `text`; `--format json` emits a pretty deterministic envelope with `type`, `round_id`, `history_path`, `node_id`, `previous_state`, `new_state`, `artifact_recorded`, nullable `artifact`, `next_ready_task_entries`, and `next_ready_tasks[]` fields, where each downstream ready task includes `role`, `kind`, `node_id`, `title`, and nullable `spec_ref`. It leaves task execution/artifact generation to the external worker and does not launch agents, schedule work, append rounds, open PRs, create cron jobs, crawl data, or write artifact files.
- `mdid-cli moat artifacts --history-path PATH [--round-id ROUND_ID] [--role planner|coder|reviewer] [--state pending|ready|in_progress|completed|blocked] [--kind market_scan|competitor_analysis|lock_in_analysis|strategy_generation|spec_planning|implementation|review|evaluation] [--node-id NODE_ID] [--contains TEXT] [--artifact-ref TEXT] [--artifact-summary TEXT] [--limit N] [--format text|json]` is a read-only inspection surface for completed task artifact handoffs persisted in the selected round's task graph. It opens only an existing history file, selects the latest persisted round unless `--round-id` exact-matches a specific persisted round, filters exact node IDs and case-sensitive raw node/ref/summary text, applies `--limit` after filtering, escapes pipe-delimited text output fields, and never mutates history, appends rounds, schedules work, launches agents, opens PRs, creates cron jobs, crawls data, or writes artifact files. `--format` defaults to `text`; `--format json` emits a pretty deterministic envelope with `type: "moat_artifacts"`, nullable `round_id`, `history_path`, `artifact_entries`, and `artifacts`, where each artifact includes node metadata (`node_id`, `node_title`, `node_role`, `node_kind`, `node_state`) plus `artifact_ref` and `artifact_summary`.
...
This shipped slice is intentionally narrower than the full autonomous moat-loop vision. It provides a deterministic local round runner over sample or explicitly supplied local JSON inputs with default text and `--format json` `moat_round` envelopes for external controllers, bounded control-plane inspection, bounded local history persistence/inspection, read-only latest-round decision-log inspection, an inspection-only continuation gate over deterministic sample data, one-shot bounded local scheduler control, bounded ready-task routing, bounded one-task dispatch envelopes, bounded `controller-step` handoff combining one ready-task selection/optional claim with work-packet context export, persisted ready-task claiming, claimed-task completion with downstream ready-task summary output, read-only artifact handoff inspection, and bounded markdown export for the latest or exact prior persisted `implemented_specs` handoff IDs such as `moat-spec/workflow-audit`. Deterministic implementation-plan markdown export now exists for those handoffs, and control-plane `agent_assignments` are read-only projections only; `ready-tasks`, `dispatch-next`, `controller-step`, `claim-task`, `heartbeat-task`, `reap-stale-tasks`, and `complete-task` provide external-controller coordination but do not launch agents, start a daemon, supervise processes, generate artifacts, or write code. Input-file `round`/`control-plane` mode reads only local files and does not crawl data, open PRs, create cron jobs, schedule background work, or write artifacts; `round` appends history only when `--history-path` is explicitly supplied. Full autonomous Planner/Coder/Reviewer process execution remains future work; there is no background scheduler/daemon, no live market crawling, and no full autonomous multi-agent runtime over user-supplied or external inputs.
