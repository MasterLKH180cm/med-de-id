...
- `mdid-cli moat task-graph --history-path PATH [--role planner|coder|reviewer] [--state pending|ready|in_progress|completed|blocked] [--node-id NODE_ID]` is a read-only latest-round inspection surface for the persisted control-plane task graph. It opens only existing history and prints `node=<role>|<node_id>|<title>|<kind>|<state>|<dependencies>|<spec_ref>` rows with `<none>` for empty dependency/spec fields, comma-joined dependency node IDs, optional role/state filtering, optional exact persisted `--node-id` matching with no normalization, and escaped pipe-delimited string fields. When no persisted node matches the combined filters, it succeeds with only the `moat task graph` header and never appends, schedules, runs agents, or launches background work. It does not mutate history, append rounds, schedule work, launch agents, crawl data, open PRs, or create cron jobs.
