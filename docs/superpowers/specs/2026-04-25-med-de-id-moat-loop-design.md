...
- `mdid-cli moat task-graph --history-path PATH [--role planner|coder|reviewer] [--state pending|ready|in_progress|completed|blocked]` is a read-only latest-round inspection surface for the persisted control-plane task graph. It opens only existing history and prints `node=<role>|<node_id>|<title>|<kind>|<state>|<dependencies>|<spec_ref>` rows with `<none>` for empty dependency/spec fields, comma-joined dependency node IDs, optional role/state filtering, and escaped pipe-delimited string fields. It does not mutate history, append rounds, schedule work, launch agents, crawl data, open PRs, or create cron jobs.
