...
- a read-only `mdid-cli moat decision-log --history-path PATH` inspection surface that requires an already-persisted history file, reads the latest persisted round without running or appending a new one, and prints each persisted decision as `decision=<role>|<summary>|<rationale>`
