...
- a bounded operator-facing `mdid-cli moat history --history-path PATH [--round-id ROUND_ID] [--decision Continue|Stop|Pivot] [--contains TEXT] [--min-score N] [--limit N]` summary surface for inspecting persisted local history, including the latest surfaced `implemented_specs` handoff IDs as `latest_implemented_specs`; `--round-id ROUND_ID` is read-only and exact-matches the persisted round ID (`entry.report.summary.round_id`, corresponding to the persisted report round ID) before `--limit`; `--min-score N` is read-only, accepts a non-negative integer, filters persisted entries where `entry.report.summary.moat_score_after >= N`, is conjunctive with `--round-id`, `--decision`, and `--contains`, applies before `--limit`, and never runs rounds, appends history, schedules work, launches agents, opens PRs, or creates cron jobs
