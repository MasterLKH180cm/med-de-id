...
- `mdid-cli moat assignments --history-path PATH [--role planner|coder|reviewer]` is a read-only latest-round inspection surface for persisted `agent_assignments`. It prints bounded `assignment=<role>|<node_id>|<title>|<kind>|<spec_ref>` rows for operators and future SDD handoff tooling; persisted string fields (`node_id`, `title`, `spec_ref`) are escaped for pipe-delimited output (`\\`, `|`, newline, and carriage return become `\\\\`, `\\|`, `\\n`, and `\\r`). It does not mutate history, schedule rounds, launch agents, crawl data, open PRs, or create cron jobs.
