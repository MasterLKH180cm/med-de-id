...
fn redact_image_ppm_writes_redacted_bytes_and_phi_safe_summary() {
    ...
    assert_eq!(summary["format"], "ppm_p6");
...
fn privacy_filter_text_detects_ip_address_from_stdin_without_raw_ip_leaks() {
    ...
    fs::create_dir(&phi_named_dir).unwrap();
    let report_path = phi_named_dir.join("privacy-filter-report.json");
    let text = "Patient Jane Example remote login from 192.168.10.42 for MRN-12345\n";
...
#[test]
fn cli_privacy_filter_text_masks_url_without_phi_or_path_leaks() {
    let dir = tempdir().unwrap();
    let phi_named_dir = dir.path().join("Jane-Example-MRN-12345-portal-url");
    fs::create_dir(&phi_named_dir).unwrap();
    let report_path = phi_named_dir.join("privacy-filter-report.json");
    let text =
        "Patient Jane Example MRN MRN-12345 portal https://portal.example.test/patient/123\n";
    ...
    assert!(stdout.contains("\"report_path\":\"<redacted>\""));
    assert_eq!(report["summary"]["category_counts"]["URL"], 1);
    assert!(report["masked_text"].as_str().unwrap().contains("[URL]"));
    assert!(report["spans"]
        .as_array()
        .unwrap()
        .iter()
...
#[test]
fn cli_privacy_filter_text_accepts_insurance_id_category_without_phi_leaks() {
    let dir = tempdir().unwrap();
    let report_path = dir.path().join("insurance-report.json");
    let runner_path = repo_path("scripts/privacy_filter/run_privacy_filter.py");
    let stdin_phi = "Patient Jane Example insurance ID ABC1234567 member number MBR-7654321\n";
    ...
    let report: Value = serde_json::from_str(&report_text).unwrap();
    assert!(stdout.contains("\"report_path\":\"<redacted>\""));
    assert_eq!(report["summary"]["category_counts"]["INSURANCE_ID"], 2);
    assert!(report["masked_text"]
        .as_str()
        .unwrap()
        .contains("[INSURANCE_ID]"));
    for span in report["spans"].as_array().unwrap() {
        assert_eq!(span["preview"], "<redacted>");
    }
    for unsafe_text in [
...
#[test]
fn cli_privacy_filter_text_accepts_age_facility_npi_and_license_plate_without_phi_leaks() {
    let dir = tempdir().unwrap();
    let report_path = dir.path().join("cli-gap-batch-report.json");
    let runner_path = repo_path("scripts/privacy_filter/run_privacy_filter.py");
    let stdin_phi = "Patient Jane Example age 87 at facility North Valley Hospital, NPI 1234567893, license plate ABC-1234\n";
    ...
    let report: Value = serde_json::from_str(&report_text).unwrap();
    assert!(stdout.contains("\"report_path\":\"<redacted>\""));
    for label in ["AGE", "FACILITY", "NPI", "LICENSE_PLATE"] {
        assert_eq!(report["summary"]["category_counts"][label], 1);
        assert!(report["masked_text"]
            .as_str()
            .unwrap()
...
fn privacy_filter_text_detects_zips_from_stdin_without_raw_zip_leaks() {
    let dir = tempdir().unwrap();
    let report_path = dir.path().join("privacy-filter-zip-report.json");
    let runner_path = repo_path("scripts/privacy_filter/run_privacy_filter.py");
    let stdin_phi = "Patient Jane Example ZIP 02139 alternate 02139-4307 MRN-12345\n";
