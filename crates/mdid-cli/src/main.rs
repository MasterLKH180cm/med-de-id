...
enum CliCommand {
    ...
    OfflineReadiness(OfflineReadinessArgs),
...
struct OfflineReadinessArgs {
    ...
    packaging_output: Option<PathBuf>,
    governance_output: Option<PathBuf>,
}
...
fn parse_command(args: &[String]) -> Result<CliCommand, String> {
    match args {
        ...
        [command, rest @ ..] if command == "offline-readiness" => {
            parse_offline_readiness_args(rest).map(CliCommand::OfflineReadiness)
        }
...
fn parse_deidentify_pdf_args(args: &[String]) -> Result<DeidentifyPdfArgs, String> {
    ...
    while index < args.len() {
        ...
        match flag {
            ...
            "--output-pdf-path" => output_pdf_path = Some(PathBuf::from(value)),
            _ => return Err("unknown flag".to_string()),
        }
        index += 2;
    }
...
fn parse_offline_readiness_args(args: &[String]) -> Result<OfflineReadinessArgs, String> {
    let mut privacy_runner_path = None;
    let mut ocr_runner_path = None;
    let mut ocr_fixture_path = None;
    let mut python_command = None;
    let mut packaging_output = None;
    let mut governance_output = None;
    ...
    while index < args.len() {
        ...
        match flag {
            ...
            "--ocr-fixture-path" => {
                ocr_fixture_path = Some(non_blank_path(value, "--ocr-fixture-path")?)
            }
            "--python-command" => {
                python_command = Some(non_blank_value(value, "--python-command")?)
            }
            "--packaging-output" => {
                packaging_output = Some(non_blank_path(value, "--packaging-output")?)
            }
            "--governance-output" => {
                governance_output = Some(non_blank_path(value, "--governance-output")?)
            }
            _ => return Err("unknown flag".to_string()),
        }
        index += 2;
    ...
    Ok(OfflineReadinessArgs {
        ...
        ocr_fixture_path: ocr_fixture_path
            .ok_or_else(|| "missing --ocr-fixture-path".to_string())?,
...
fn parse_ocr_privacy_evidence_args(args: &[String]) -> Result<OcrPrivacyEvidenceArgs, String> {
    ...
    let mut runner_path = None;
    let mut output_path = None;
    let mut summary_output_path = None;
    let mut python_command = None;
    let mut mock = false;
    let mut index = 0;
    while index < args.len() {
        ...
        match flag {
            ...
            "--summary-output" => {
                summary_output_path = Some(non_blank_path(value, "--summary-output")?)
            }
            "--python-command" => {
                if value.trim().is_empty() {
                    return Err("missing --python-command".to_string());
...
fn non_blank_value(value: &str, flag: &str) -> Result<String, String> {
    if value.trim().is_empty() {
        return Err(format!("missing {flag}"));
    }
    Ok(value.to_string())
...
fn paths_equivalent_for_output(left: &Path, right: &Path) -> Result<bool, String> {
    Ok(normalized_output_path(left)? == normalized_output_path(right)?)
}

fn normalized_output_path(path: &Path) -> Result<PathBuf, String> {
    let mut normalized = if path.is_absolute() {
        PathBuf::new()
    } else {
        std::env::current_dir().map_err(|_| "failed to validate output paths".to_string())?
    };
...
fn run_command(command: CliCommand) -> Result<(), String> {
    match command {
        ...
        CliCommand::OfflineReadiness(args) => run_offline_readiness(args),
...
fn run_redact_image_ppm(args: RedactImagePpmArgs) -> Result<(), String> {
    ...
    let (redacted, verification) =
        redact_ppm_p6_bytes_with_verification(&input, &regions, [0, 0, 0])
...
fn run_offline_readiness(args: OfflineReadinessArgs) -> Result<(), String> {
    let report = build_offline_readiness_report(&args)?;
    if let Some(packaging_output) = &args.packaging_output {
        write_json_artifact(
            packaging_output,
            &report["packaging_hardening"],
            "offline readiness packaging evidence",
        )?;
    }
    if let Some(governance_output) = &args.governance_output {
        write_json_artifact(
            governance_output,
            &report["audit_governance"],
            "offline readiness governance evidence",
        )?;
    }
    println!(
        "{}",
        serde_json::to_string(&report)
            .map_err(|err| format!("failed to render offline readiness report: {err}"))?
    );
    Ok(())
}

fn write_json_artifact(path: &Path, value: &Value, context: &str) -> Result<(), String> {
    let rendered = serde_json::to_string_pretty(value)
        .map_err(|err| format!("failed to render {context}: {err}"))?;
    fs::write(path, rendered).map_err(|_| format!("failed to write {context}"))
}

fn build_offline_readiness_report(args: &OfflineReadinessArgs) -> Result<Value, String> {
    require_regular_file(
        &args.privacy_runner_path,
        "missing Privacy Filter runner file",
    )?;
    ...
    let python_configured = !args.python_command.trim().is_empty();
    ...
    Ok(json!({
        ...
        "python_command_configured": python_configured,
        "privacy_filter": {
        ...
        "packaging_hardening": {
            "manifest_output_requested": args.packaging_output.is_some(),
            "install_manifest_scope": "local_cli_usage_evidence",
            "raw_paths_included": false,
            "network_required": false,
            "portable_binary_claimed": false,
            "scope": "repository_bounded_cli_packaging_evidence"
        },
        "audit_governance": {
            "summary_output_requested": args.governance_output.is_some(),
            "scope": "aggregate_local_cli_governance_evidence",
            "raw_values_included": false,
            "raw_paths_included": false,
            "audit_surface": "vault_audit_summary_and_cli_artifact_readiness"
        },
        "non_goals": [
            "network_api_use",
...
fn run_ocr_privacy_evidence(args: OcrPrivacyEvidenceArgs) -> Result<(), String> {
    if let Some(summary_output_path) = &args.summary_output_path {
        if paths_are_same_existing_or_lexical(&args.output_path, summary_output_path) {
            return Err(
                "ocr privacy evidence summary path must differ from report path".to_string(),
            );
        }
    }
    let _ = fs::remove_file(&args.output_path);
    if let Some(summary_output_path) = &args.summary_output_path {
        let _ = fs::remove_file(summary_output_path);
    }
    let result = (|| {
        ...
        fs::write(&args.output_path, rendered)
            .map_err(|_| "OCR privacy evidence failed".to_string())?;
        let summary_written = if let Some(summary_output_path) = &args.summary_output_path {
            let summary = build_ocr_privacy_evidence_summary(&value)?;
            let summary_rendered = format!(
                "{}\n",
                serde_json::to_string_pretty(&summary)
                    .map_err(|_| "OCR privacy evidence failed".to_string())?
            );
            if summary_rendered.len() > OCR_PRIVACY_EVIDENCE_REPORT_MAX_BYTES {
                return Err("OCR privacy evidence failed".to_string());
            }
            fs::write(summary_output_path, summary_rendered)
                .map_err(|_| "OCR privacy evidence failed".to_string())?;
            true
        } else {
            false
        };
        let mut stdout = json!({"command":"ocr-privacy-evidence","artifact":"ocr_privacy_evidence","report_path":"<redacted>","report_written":true,"ocr_scope":"printed_text_line_extraction_only","privacy_scope":"text_only_pii_detection","network_api_called":false});
        if summary_written {
            stdout["summary_written"] = json!(true);
        }
        println!(
            "{}",
            serde_json::to_string(&stdout).map_err(|_| "OCR privacy evidence failed".to_string())?
        );
        Ok(())
    })();
    if result.is_err() {
...
fn build_ocr_privacy_evidence_summary(report: &Value) -> Result<Value, String> {
    let mut category_counts = serde_json::Map::new();
    for category in ["NAME", "MRN", "EMAIL", "PHONE", "ID"] {
        if let Some(count) = report["category_counts"][category].as_u64() {
...
fn run_deidentify_pdf(args: DeidentifyPdfArgs) -> Result<(), String> {
    if let Some(output_pdf_path) = args.output_pdf_path.as_ref() {
        if paths_equivalent_for_output(&args.report_path, output_pdf_path)? {
            return Err("report and output PDF paths must be different".to_string());
        }
    }
...
mod tests {
    ...
    fn parses_offline_readiness_command_for_cli_opf_and_ocr_evidence() {
        let command = parse_command(&[
            ...
            "--packaging-output".to_string(),
            "/tmp/package-manifest.json".to_string(),
            "--governance-output".to_string(),
            "/tmp/governance-summary.json".to_string(),
        ])
        ...
        match command {
            CliCommand::OfflineReadiness(args) => {
                assert_eq!(
                    args.privacy_runner_path,
                    PathBuf::from("scripts/privacy_filter/run_privacy_filter.py")
                );
                assert_eq!(
                    args.ocr_runner_path,
                    PathBuf::from("scripts/ocr_eval/run_small_ocr.py")
                );
                assert_eq!(
                    args.ocr_fixture_path,
                    PathBuf::from("scripts/ocr_eval/fixtures/synthetic_printed_phi_line.png")
                );
                assert_eq!(
                    args.packaging_output,
                    Some(PathBuf::from("/tmp/package-manifest.json"))
                );
                assert_eq!(
                    args.governance_output,
                    Some(PathBuf::from("/tmp/governance-summary.json"))
                );
            }
    ...
    #[test]
    fn offline_readiness_report_is_aggregate_only_and_marks_opf_ocr_local_paths() {
        let temp_dir = tempfile::tempdir().expect("temp dir");
        let privacy_runner = temp_dir.path().join("run_privacy_filter.py");
        let ocr_runner = temp_dir.path().join("run_small_ocr.py");
        let ocr_fixture = temp_dir.path().join("synthetic_printed_phi_line.png");
        std::fs::write(&privacy_runner, "# mock privacy runner").expect("privacy runner");
        std::fs::write(&ocr_runner, "# mock ocr runner").expect("ocr runner");
        std::fs::write(&ocr_fixture, b"fixture").expect("ocr fixture");

        let report = build_offline_readiness_report(&OfflineReadinessArgs {
            privacy_runner_path: privacy_runner,
            ocr_runner_path: ocr_runner,
            ocr_fixture_path: ocr_fixture,
            python_command: default_python_command(),
            packaging_output: Some(temp_dir.path().join("package-manifest.json")),
            governance_output: Some(temp_dir.path().join("governance-summary.json")),
        })
        .expect("offline readiness report");
        let text = serde_json::to_string(&report).expect("json");

        assert_eq!(report["artifact"], "offline_cli_ocr_readiness");
        assert_eq!(report["cli_surface"], "ready");
        assert_eq!(
            report["privacy_filter"]["default_mode"],
        ...
        assert_eq!(report["ocr"]["scope"], "printed_text_line_extraction_only");
        assert_eq!(
        ...
        assert_eq!(
            report["packaging_hardening"]["install_manifest_scope"],
            "local_cli_usage_evidence"
        );
        ...
        assert_eq!(
            report["audit_governance"]["scope"],
            "aggregate_local_cli_governance_evidence"
        );
        assert!(!text.contains(temp_dir.path().to_string_lossy().as_ref()));
