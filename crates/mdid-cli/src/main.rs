...
fn run_ocr_handoff(args: OcrHandoffArgs) -> Result<(), String> {
    ...
    let result = (|| {
        ...
        let report_path = if args.summary_output.is_some() {
            json!("<redacted>")
        } else {
            json!(args.report_path)
        };
        let summary = json!({
            "command": "ocr-handoff",
