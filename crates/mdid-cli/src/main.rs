...
impl Default for MoatOutputFormat {
    fn default() -> Self {
        Self::Text
    }
}
...
struct MoatReadyTasksCommand {
    ...
    spec_ref: Option<String>,
    limit: Option<usize>,
    output_format: MoatOutputFormat,
}
...
fn main() {
    ...
    match parse_command(&args) {
        ...
        Ok(CliCommand::MoatWorkPacket(command)) => {
            if let Err(error) = run_moat_work_packet(&command) {
                exit_with_error(error);
            }
        }
        ...
        Ok(CliCommand::MoatScheduleNext {
            ...
            if let Err(error) = run_moat_schedule_next(&history_path, improvement_threshold, format)
            {
                exit_with_error(error);
            }
        }
...
enum CliCommand {
    ...
    MoatWorkPacket(MoatWorkPacketCommand),
...
fn parse_command(args: &[String]) -> Result<CliCommand, String> {
    match args {
        ...
        [moat, work_packet, rest @ ..] if moat == "moat" && work_packet == "work-packet" => Ok(
            CliCommand::MoatWorkPacket(parse_moat_work_packet_command(rest)?),
        ),
    ...
    while index < args.len() {
        if args[index] == "--format" {
            let value = args
                ...
                .ok_or_else(|| "missing value for moat round --format".to_string())?;
            if format.is_some() {
                return Err("duplicate moat round --format".to_string());
            }
            format = Some(parse_moat_round_format(value)?);
            index += 2;
        } else {
            round_args.push(args[index].clone());
            if let Some(value) = args.get(index + 1) {
                round_args.push(value.clone());
            }
            index += 2;
        }
    }
    let (overrides, history_path, input_path) =
        parse_moat_round_overrides(&round_args, true, "moat round")?;
    Ok(MoatRoundCommand {
        overrides,
        history_path,
        input_path,
...
fn parse_moat_round_format(value: &str) -> Result<MoatOutputFormat, String> {
    match value {
        "text" => Ok(MoatOutputFormat::Text),
        "json" => Ok(MoatOutputFormat::Json),
        other => Err(format!("unknown moat round format: {other}")),
    }
}
...
fn parse_moat_control_plane_command(args: &[String]) -> Result<MoatControlPlaneCommand, String> {
    ...
    while index < args.len() {
        if args[index] == "--format" {
            let value = args
                .get(index + 1)
                .filter(|value| !value.starts_with("--"))
                .ok_or_else(|| "missing value for moat control-plane --format".to_string())?;
            if format.is_some() {
                return Err("duplicate moat control-plane --format".to_string());
            }
            format = Some(parse_moat_control_plane_format(value)?);
            index += 2;
        } else {
            round_args.push(args[index].clone());
            if let Some(value) = args.get(index + 1) {
    ...
    let (overrides, history_path, input_path) =
        parse_moat_round_overrides(&round_args, true, "moat control-plane")?;
    if history_path.is_some()
        && (overrides != MoatRoundOverrides::default() || input_path.is_some())
    {
        return Err(
...
fn parse_moat_control_plane_format(value: &str) -> Result<MoatOutputFormat, String> {
    match value {
        "text" => Ok(MoatOutputFormat::Text),
        "json" => Ok(MoatOutputFormat::Json),
        other => Err(format!("unknown moat control-plane format: {other}")),
    }
}
...
fn parse_moat_history_command(args: &[String]) -> Result<MoatHistoryCommand, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--format" => {
                let value = required_flag_value(args, index, "--format", false)?;
                if format.is_some() {
                    return Err(duplicate_flag_error("--format"));
                }
                format = Some(parse_moat_history_format(value)?);
                index += 2;
            }
...
fn parse_moat_history_format(value: &str) -> Result<MoatOutputFormat, String> {
    match value {
        "text" => Ok(MoatOutputFormat::Text),
        "json" => Ok(MoatOutputFormat::Json),
        other => Err(format!(
            "error: unknown output format for --format: {other}"
        )),
    }
}
...
fn parse_moat_assignments_command(args: &[String]) -> Result<MoatAssignmentsCommand, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--assigned-agent-id" => {
                let value = required_flag_value(args, index, "--assigned-agent-id", false)?;
                if assigned_agent_id.is_some() {
                    return Err(duplicate_flag_error("--assigned-agent-id"));
                }
                assigned_agent_id = Some(value.clone());
            }
            "--depends-on" => {
...
fn parse_moat_task_events_command(args: &[String]) -> Result<MoatTaskEventsCommand, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--format" => {
                let value = required_flag_value(args, index, "--format", true)
                    .map_err(|_| "missing value for moat task-events --format".to_string())?;
                if format.is_some() {
                    return Err("duplicate moat task-events --format".to_string());
                }
                format = Some(parse_moat_task_events_format(value)?);
            }
            flag => return Err(format!("unknown flag: {flag}")),
        }
        index += 2;
...
fn parse_moat_task_events_format(value: &str) -> Result<MoatOutputFormat, String> {
    match value {
        "text" => Ok(MoatOutputFormat::Text),
        "json" => Ok(MoatOutputFormat::Json),
        other => Err(format!("unknown moat task-events format: {other}")),
    }
}
        ...
        match args[index].as_str() {
            ...
            "--requires-artifacts" => {
                if requires_artifacts {
                    return Err(duplicate_flag_error("--requires-artifacts"));
                }
            ...
            "--format" => {
                let value = args
                    .get(index + 1)
                    .filter(|value| !value.starts_with("--"))
                    .ok_or_else(|| "missing value for moat ready-tasks --format".to_string())?;
                if output_format.is_some() {
                    return Err("duplicate moat ready-tasks --format".to_string());
                }
                output_format = Some(match value.as_str() {
                    "text" => MoatOutputFormat::Text,
                    "json" => MoatOutputFormat::Json,
                    other => return Err(format!("unknown moat ready-tasks format: {other}")),
                });
            }
            flag => return Err(format!("unknown flag: {flag}")),
        }
...
fn parse_moat_work_packet_command(args: &[String]) -> Result<MoatWorkPacketCommand, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--history-path" => {
                let value = required_history_path_value(args, index)?.clone();
                if history_path.is_some() {
                    return Err("duplicate moat work-packet --history-path".to_string());
                }
                history_path = Some(value);
            }
            "--round-id" => {
            ...
            "--node-id" => {
                let value = required_flag_value(args, index, "--node-id", true)?;
                if node_id.is_some() {
                    return Err("duplicate moat work-packet --node-id".to_string());
                }
                node_id = Some(value.to_string());
            }
            "--format" => {
                let value = args
                    .get(index + 1)
                    .filter(|value| !value.starts_with("--"))
                    .ok_or_else(|| "missing value for moat work-packet --format".to_string())?;
                if format.is_some() {
                    return Err("duplicate moat work-packet --format".to_string());
                }
                format = Some(match value.as_str() {
                    "text" => MoatOutputFormat::Text,
                    "json" => MoatOutputFormat::Json,
                    other => return Err(format!("unknown moat work-packet format: {other}")),
                });
            }
            flag => return Err(format!("unknown flag: {flag}")),
        }
    ...
    Ok(MoatWorkPacketCommand {
        history_path: history_path
            .ok_or_else(|| "missing required moat work-packet --history-path".to_string())?,
        round_id,
        node_id: node_id
            .ok_or_else(|| "missing required moat work-packet --node-id".to_string())?,
        format: format.unwrap_or(MoatOutputFormat::Text),
    })
}
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--format" => {
                ...
                format = Some(parse_moat_artifacts_format(value)?);
            }
            flag => return Err(format!("unknown flag: {flag}")),
        }
...
fn parse_moat_artifacts_format(value: &str) -> Result<MoatOutputFormat, String> {
    match value {
        "text" => Ok(MoatOutputFormat::Text),
        "json" => Ok(MoatOutputFormat::Json),
        other => Err(format!("unknown moat artifacts format: {other}")),
    }
}
...
fn parse_moat_dispatch_next_command(args: &[String]) -> Result<MoatDispatchNextCommand, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--agent-id" => {
                let value = required_flag_value(args, index, "--agent-id", true)?;
                if agent_id.is_some() {
                    return Err(duplicate_flag_error("--agent-id"));
                }
            ...
            "--node-id" => {
                let value = args
                    .get(index + 1)
                    .filter(|value| !value.starts_with("--"))
                    .ok_or_else(|| "missing value for moat dispatch-next --node-id".to_string())?;
                if node_id.is_some() {
...
fn parse_moat_controller_step_command(
    ...
    let command = parse_moat_dispatch_next_command(args).map_err(|error| {
        error
            .replace(
            ...
            .replace(
                "missing value for --history-path",
                "missing value for moat controller-step --history-path",
            )
            .replace("moat dispatch-next", "moat controller-step")
            .replace("dispatch-next", "controller-step")
            .replace(
                "unknown moat controller-step role",
                "unknown moat task role",
            )
            .replace(
                "unknown moat controller-step kind",
                "unknown moat task kind",
            )
            .replace(
                "missing value for --round-id",
            ...
            .replace(
                "--depends-on and --no-dependencies cannot be combined",
                "moat controller-step cannot combine --depends-on and --no-dependencies",
            )
            .replace("unknown flag:", "unknown option for moat controller-step:")
            .replace(
                "invalid moat controller-step --lease-seconds",
                "moat controller-step --lease-seconds must be positive",
            )
    })?;
    if command.depends_on.is_some() && command.no_dependencies {
        return Err(
            "moat controller-step cannot combine --depends-on and --no-dependencies".to_string(),
        );
    }
    Ok(command)
}

fn parse_dispatch_output_format(value: &str) -> Result<DispatchOutputFormat, String> {
    match value {
        "text" => Ok(DispatchOutputFormat::Text),
        "json" => Ok(DispatchOutputFormat::Json),
        _ => Err(format!("unknown moat dispatch-next format: {value}")),
    }
}
...
fn parse_moat_claim_task_command(args: &[String]) -> Result<MoatClaimTaskCommand, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--format" => {
                if format.is_some() {
                    return Err("duplicate moat lifecycle --format".to_string());
                }
                let value = args
                    .get(index + 1)
                    .filter(|value| !value.starts_with("--"))
                    .ok_or_else(|| "missing value for moat lifecycle --format".to_string())?;
                format = Some(match value.as_str() {
                    "text" => MoatOutputFormat::Text,
                    "json" => MoatOutputFormat::Json,
                    other => return Err(format!("unknown moat lifecycle format: {other}")),
                });
            }
            flag => return Err(format!("unknown flag: {flag}")),
        }
...
fn parse_moat_reap_stale_tasks_command(
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--now" => {
                let value = required_flag_value(args, index, "--now", true)?;
                if now.is_some() {
                    return Err(duplicate_flag_error("--now"));
                }
        ...
        match args[index].as_str() {
            ...
            "--format" => {
                ...
                format = Some(match value.as_str() {
                    ...
                    other => {
                        return Err(format!(
                            "\u{0}NO_USAGEerror: unknown output format for --format: {other}"
                        ));
                    }
                });
            }
...
fn parse_moat_round_overrides(
    ...
    command_name: &str,
) -> Result<(MoatRoundOverrides, Option<String>, Option<PathBuf>), String> {
        ...
        match flag.as_str() {
            ...
            "--input-path" => {
                let value = args
                    .get(index + 1)
                    .filter(|value| !value.starts_with("--"))
                    .ok_or_else(|| format!("missing value for {command_name} --input-path"))?;
                if input_path.replace(PathBuf::from(value)).is_some() {
                    return Err(format!("duplicate {command_name} --input-path"));
                }
            }
    ...
    Ok((overrides, history_path, input_path))
}
...
fn run_moat_round(command: &MoatRoundCommand) -> Result<(), String> {
    let report = run_bounded_round(round_input_for_command(command)?);
    ...
    match command.format {
        MoatOutputFormat::Text => print_moat_round_text(command, &report),
        MoatOutputFormat::Json => print_moat_round_json(command, &report),
    }

    Ok(())
}

fn print_moat_round_text(command: &MoatRoundCommand, report: &MoatRoundReport) {
    ...
    if let Some(input_path) = &command.input_path {
...
fn print_moat_round_json(command: &MoatRoundCommand, report: &MoatRoundReport) {
    let control_plane = &report.control_plane;
    let assignments: Vec<String> = control_plane
        .agent_assignments
        .iter()
        .map(|assignment| {
            format!(
        ...
        .collect();
    let task_states: Vec<String> = control_plane
        .task_graph
        .nodes
        .iter()
        .map(|node| format!("{}:{}", node.node_id, format_task_node_state(node.state)))
        .collect();
    let decision_summary = control_plane
        .memory
        .latest_decision_summary()
        .unwrap_or_else(|| "<none>".to_string());
    let envelope = serde_json::json!({
        "type": "moat_round",
        "source": if command.input_path.is_some() { "input" } else { "sample" },
        "history_path": command.history_path,
        "input_path": command.input_path.as_ref().map(|path| path.display().to_string()),
        "history_saved": command.history_path.is_some(),
        "round_id": report.summary.round_id,
        "continue_decision": format_continue_decision(report.summary.continue_decision),
        "executed_tasks": report.executed_tasks,
        "implemented_specs": report.summary.implemented_specs,
        "moat_score_before": report.summary.moat_score_before,
        "moat_score_after": report.summary.moat_score_after,
        "improvement_delta": control_plane.memory.improvement_delta,
        "stop_reason": report.stop_reason,
        "ready_tasks": control_plane.task_graph.ready_node_ids(),
    ...
    println!(
        "{}",
        serde_json::to_string_pretty(&envelope).expect("moat round envelope serializes")
    );
}
...
fn run_moat_control_plane(command: &MoatControlPlaneCommand) -> Result<(), String> {
    if let Some(history_path) = &command.history_path {
        return run_persisted_moat_control_plane(history_path, command.format);
    }

    let report = run_bounded_round(control_plane_input_for_command(command)?);
    let source = if command.input_path.is_some() {
        "input"
    } else {
        "sample"
    };
...
fn run_persisted_moat_control_plane(
    history_path: &str,
    format: MoatOutputFormat,
) -> Result<(), String> {
...
fn print_control_plane_snapshot(
    ...
    let control_plane = &report.control_plane;
    let ready_node_ids = control_plane.task_graph.ready_node_ids();
    let ready_nodes = format_ready_nodes(&ready_node_ids);
    ...
    if format == MoatOutputFormat::Json {
        let assignments: Vec<String> = control_plane
            .agent_assignments
            .iter()
            .map(|assignment| {
            ...
            .collect();
        let task_states_json: Vec<serde_json::Value> = control_plane
            .task_graph
            .nodes
            .iter()
            .map(|node| {
            ...
            .collect();
        let envelope = serde_json::json!({
            "type": "moat_control_plane",
            "history_path": history_path,
            "source": source,
            "round_id": report.summary.round_id,
            "score": report.summary.moat_score_after,
            "improvement_delta": control_plane.memory.improvement_delta,
            "can_continue": report.summary.continue_decision == ContinueDecision::Continue,
            "ready_tasks": ready_node_ids,
            "assignments": assignments,
            "task_states": task_states_json,
            "decision_summary": latest_decision_summary,
            "constraints": ["local_only", "read_only", "no_agent_launch", "no_daemon", "no_pr_creation", "no_cron_creation"],
        });
        println!(
            "{}",
            serde_json::to_string_pretty(&envelope).expect("control-plane envelope serializes")
        );
        return;
    }
...
fn run_moat_history(command: &MoatHistoryCommand) -> Result<(), String> {
    ...
    if command.format == MoatOutputFormat::Json {
        print_moat_history_json(&command.history_path, &filtered_entries)?;
        return Ok(());
    }
...
fn print_moat_history_json(
    history_path: &str,
    entries: &[&MoatHistoryEntry],
) -> Result<(), String> {
    let latest = entries.last();
    let best_moat_score_after = entries
        .iter()
    ...
    let rounds = entries
        ...
        .map(|entry| {
            ...
            serde_json::json!({
                "round_id": summary.round_id.to_string(),
                "decision": format_continue_decision(summary.continue_decision),
                "moat_score_after": summary.moat_score_after,
                "stop_reason": summary.stop_reason,
                "tests_passed": summary.tests_passed,
                "implemented_specs": summary.implemented_specs,
            })
        })
        .collect::<Vec<_>>();
    let envelope = serde_json::json!({
        ...
        "summary": {
            ...
            "latest_round_id": latest.map(|entry| entry.report.summary.round_id.to_string()),
            "latest_decision": latest.map(|entry| format_continue_decision(entry.report.summary.continue_decision)),
            "latest_implemented_specs": latest
                .map(|entry| entry.report.summary.implemented_specs.clone())
                .unwrap_or_default(),
            "latest_moat_score_after": latest.map(|entry| entry.report.summary.moat_score_after),
            "best_moat_score_after": best_moat_score_after,
        },
        "history_rounds": entries.len(),
        "rounds": rounds,
    });
    println!(
        "{}",
            ...
            command
                ...
                .map(|expected| {
                    latest
                        ...
                        .iter()
                        .find(|node| node.node_id == assignment.node_id)
                        .map(|node| node.assigned_agent_id.as_deref() == Some(expected))
                        .unwrap_or(false)
                })
                .unwrap_or(true)
        })
    ...
    for node in latest
        ...
        .filter(|node| {
            command
                .assigned_agent_id
                .as_deref()
                .map(|expected| node.assigned_agent_id.as_deref() == Some(expected))
                .unwrap_or(true)
        })
        .filter(|node| {
            command
...
fn run_moat_task_events(command: &MoatTaskEventsCommand) -> Result<(), String> {
    ...
    let Some(entry) = selected else {
        if command.format == MoatOutputFormat::Json {
            print_moat_task_events_json(command, None, &[])?;
        } else {
    ...
    if command.format == MoatOutputFormat::Json {
        print_moat_task_events_json(
            command,
            Some(entry.report.summary.round_id.to_string()),
            &events,
        )?;
        return Ok(());
    }
...
fn print_moat_task_events_json(
    command: &MoatTaskEventsCommand,
    round_id: Option<String>,
    events: &[&mdid_domain::MoatTaskEvent],
) -> Result<(), String> {
    let event_values = events
        ...
        .map(|event| {
            serde_json::json!({
                ...
                "action": format_moat_task_event_action(event.action),
                ...
                "artifact_summary": serde_json::Value::Null,
    ...
    let envelope = serde_json::json!({
        ...
        "round_id": round_id,
        "history_path": command.history_path,
        "task_event_entries": event_values.len(),
        "events": event_values,
    });
    println!(
        "{}",
        serde_json::to_string_pretty(&envelope)
            .map_err(|error| format!("failed to serialize task events envelope: {error}"))?
    );
    Ok(())
}
...
fn run_moat_artifacts(command: &MoatArtifactsCommand) -> Result<(), String> {
    ...
    if command.format == MoatOutputFormat::Text {
        println!("moat artifacts");
    }
    let Some(entry) = selected else {
        match command.format {
            MoatOutputFormat::Text => println!("artifact_entries=0"),
            MoatOutputFormat::Json => {
                let envelope = serde_json::json!({
                    "type": "moat_artifacts",
                    "round_id": serde_json::Value::Null,
                    "history_path": command.history_path,
                    "artifact_entries": 0,
                    "artifacts": [],
                });
                println!(
                    "{}",
                    serde_json::to_string_pretty(&envelope)
                        .map_err(|error| format!("failed to serialize artifacts json: {error}"))?
                );
            }
        }
        return Ok(());
    ...
    let mut artifacts = entry
        ...
        .filter(|node| command.kind.map(|kind| node.kind == kind).unwrap_or(true))
        .flat_map(|node| node.artifacts.iter().map(move |artifact| (node, artifact)))
        .filter(|(node, _artifact)| {
            command
                .node_id
                .as_deref()
                .map(|expected| node.node_id == expected)
                .unwrap_or(true)
        })
        .filter(|(node, artifact)| {
            command
                .contains
                .as_deref()
                .map(|needle| {
    ...
    match command.format {
        ...
        MoatOutputFormat::Json => {
            let artifacts_json = artifacts
                ...
                .map(|(node, artifact)| {
                    serde_json::json!({
                        ...
                        "node_role": format_agent_role(node.role),
                        "node_kind": format_moat_task_kind(node.kind),
                        "node_state": format_task_node_state(node.state),
            ...
            let envelope = serde_json::json!({
                ...
                "round_id": round_id,
                "history_path": command.history_path,
                "artifact_entries": artifacts_json.len(),
...
fn run_moat_dispatch_next(command: &MoatDispatchNextCommand) -> Result<(), String> {
    ...
    if command.output_format == DispatchOutputFormat::Json {
        let mut envelope = serde_json::json!({
            ...
            "complete_command": format_dispatch_complete_command(command, &round_id, &selected.node_id),
        });
        if !command.dry_run {
        ...
        println!(
            "{}",
            serde_json::to_string_pretty(&envelope)
                .map_err(|error| format!("failed to serialize dispatch envelope: {error}"))?
        );
        return Ok(());
    }
...
fn run_moat_controller_step(command: &MoatControllerStepCommand) -> Result<(), String> {
    ...
    let assigned_agent_id = (!command.dry_run)
        .then(|| command.agent_id.as_deref())
        .flatten();
    let complete_command = format_dispatch_complete_command(command, &round_id, &selected.node_id);
    let acceptance = [
    ...
    println!(
        "assigned_agent_id={}",
        assigned_agent_id.unwrap_or("<none>")
    );
    println!("round_id={round_id}");
    println!("node_id={}", selected.node_id);
    println!("role={}", format_agent_role(selected.role));
    ...
    println!("complete_command={complete_command}");
            ...
            ready_ids.iter().any(|ready_id| ready_id == &node.node_id)
                ...
                && command
                    ...
                    .map(|dependency| {
                        node.depends_on
                            .iter()
                            .any(|node_dependency| node_dependency == dependency)
                    })
                    .unwrap_or(true)
...
fn dependency_artifact_requirements_satisfied(
    candidate: &MoatTaskNode,
    all_nodes: &[MoatTaskNode],
) -> bool {
    candidate.depends_on.iter().all(|dependency_id| {
        all_nodes.iter().any(|node| {
            node.node_id == *dependency_id
                && node.state == MoatTaskNodeState::Completed
                && !node.artifacts.is_empty()
        })
    })
}
...
fn format_dispatch_complete_command(
    ...
    if let Some(agent_id) = command.agent_id.as_deref() {
        parts.extend(["--agent-id".to_string(), shell_single_quote(agent_id)]);
    }
    parts.extend([
    ...
    match command.format {
        ...
        MoatOutputFormat::Json => {
            let envelope = serde_json::json!({
                ...
                "lease_expires_at": (now + chrono::Duration::seconds(command.lease_seconds)).to_rfc3339(),
...
fn run_moat_work_packet(command: &MoatWorkPacketCommand) -> Result<(), String> {
    let store = LocalMoatHistoryStore::open_existing(&command.history_path)
            ...
            nodes
                ...
                .flat_map(move |dependency| {
                    dependency
                        .artifacts
                        .iter()
                        .map(move |artifact| (dependency, artifact))
                })
        })
        .collect::<Vec<_>>();
    let acceptance = [
    ...
    let complete_command = format!(
        "mdid-cli moat complete-task --history-path {} --round-id {} --node-id {} --artifact-ref {} --artifact-summary {}",
        shell_single_quote(&command.history_path),
        shell_single_quote(&entry.report.summary.round_id.to_string()),
        shell_single_quote(&command.node_id),
        shell_single_quote("<artifact-ref>"),
        shell_single_quote("<artifact-summary>")
    );
    match command.format {
        MoatOutputFormat::Text => {
            println!("moat work packet");
            println!("round_id={}", entry.report.summary.round_id);
            println!("history_path={}", command.history_path);
            println!("node_id={}", node.node_id);
            println!("title={}", node.title);
            println!("role={}", format_agent_role(node.role));
            println!("kind={}", format_moat_task_kind(node.kind));
            println!("state={}", format_task_node_state(node.state));
            println!("spec_ref={}", node.spec_ref.as_deref().unwrap_or("<none>"));
            for dependency in &node.depends_on {
                println!("dependency={dependency}");
            }
            for (dependency, artifact) in &dependency_artifacts {
                println!(
                    "dependency_artifact={}|{}|{}",
                    escape_assignment_output_field(&dependency.node_id),
                    escape_assignment_output_field(&artifact.artifact_ref),
                    escape_assignment_output_field(&artifact.summary)
                );
            }
        ...
        MoatOutputFormat::Json => {
            let artifacts = dependency_artifacts
                .iter()
            ...
            println!(
                "{}",
                serde_json::to_string_pretty(&envelope)
                    .map_err(|error| format!("failed to serialize moat work packet: {error}"))?
            );
        }
    }
    Ok(())
...
fn run_moat_ready_tasks(command: &MoatReadyTasksCommand) -> Result<(), String> {
    ...
    let Some(entry) = selected else {
        match command.output_format {
            ...
            MoatOutputFormat::Json => {
                let envelope = serde_json::json!({
                    "type": "moat_ready_tasks",
                    "round_id": command.round_id.as_deref().unwrap_or(""),
                    "history_path": command.history_path,
                ...
                println!(
                    "{}",
                    serde_json::to_string_pretty(&envelope).map_err(|error| format!(
                        "failed to serialize moat ready tasks: {error}"
                    ))?
                );
            }
        }
    ...
    match command.output_format {
        ...
        MoatOutputFormat::Json => {
            ...
            let envelope = serde_json::json!({
                ...
                "round_id": entry.report.summary.round_id.to_string(),
                "history_path": command.history_path,
                "ready_task_entries": tasks.len(),
                "tasks": tasks,
            });
            println!(
                "{}",
                serde_json::to_string_pretty(&envelope)
                    .map_err(|error| format!("failed to serialize moat ready tasks: {error}"))?
            );
        }
    }
    ...
    match format {
        ...
        MoatOutputFormat::Json => {
            let envelope = serde_json::json!({
                ...
                "history_path": history_path,
                "scheduled": gate.can_continue,
                "reason": gate.reason,
                "scheduled_round_id": scheduled_round_id,
                "required_improvement_threshold": gate.required_improvement_threshold,
            });
            println!(
                "{}",
                serde_json::to_string_pretty(&envelope).map_err(|error| format!(
                    "failed to serialize moat schedule-next json: {error}"
                ))?
            );
        }
    }
...
fn control_plane_input_for_command(
    command: &MoatControlPlaneCommand,
) -> Result<MoatRoundInput, String> {
    let mut input = if let Some(path) = &command.input_path {
        load_round_input(path)?
    } else {
        sample_round_input(&MoatRoundOverrides::default())
...
fn load_round_input(path: &Path) -> Result<MoatRoundInput, String> {
    let content = fs::read_to_string(path).map_err(|error| {
        format!(
            "failed to read moat round input {}: {error}",
            path.display()
        )
    })?;
    serde_json::from_str(&content).map_err(|error| {
        format!(
            "failed to parse moat round input {}: {error}",
            path.display()
        )
    })
}
...
fn sample_round_input(overrides: &MoatRoundOverrides) -> MoatRoundInput {
    ...
    apply_round_overrides(&mut input, overrides);
...
mod tests {
    ...
    #[test]
    fn parse_moat_history_command_parses_json_format() {
        assert_eq!(
            parse_moat_history_command(&[
                "--history-path".into(),
                "history.json".into(),
